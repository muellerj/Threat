...
function Prot()
	if (Prot_Configuration["Enabled"] and not UnitIsCivilian("target") and UnitClass("player") == CLASS_WARRIOR_PROT and ProtTalents) then
		...
		if (ProtOverpower) then
			if ((GetTime() - ProtOverpower) > 4) then
				ProtOverpower = nil;
			end
		end

		if (ProtSpellInterrupt) then
			if ((GetTime() - ProtSpellInterrupt) > 2) then
		...
		elseif (Prot_Configuration[ABILITY_BERSERKER_RAGE_PROT]
			and (ProtIncapacitate or ProtFear)
			and ActiveStance() == 3
			and SpellReady(ABILITY_BERSERKER_RAGE_PROT)) then
			Debug("Berserker Rage");
			CastSpellByName(ABILITY_BERSERKER_RAGE_PROT);
			ProtLastSpellCast = GetTime();

		elseif (Prot_Configuration[ABILITY_EXECUTE_PROT]
			and Weapon()
			and UnitMana("player") >= ProtExecuteCost
			and (ActiveStance() ~= 2 or (UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"]) and Prot_Configuration["PrimaryStance"] ~= 0))
			and (UnitHealth("target") / UnitHealthMax("target") * 100) < 20
			and SpellReady(ABILITY_EXECUTE_PROT)) then
			if (ActiveStance() ~= 2) then
				Debug("Execute");
				if (ProtOldStance == 2) then
					ProtDanceDone = true;
				end
				CastSpellByName(ABILITY_EXECUTE_PROT);
				ProtLastSpellCast = GetTime();
			else
				if (ProtLastStanceCast + 1.5 <= GetTime()) then
					if (not ProtOldStance) then
						ProtOldStance = ActiveStance();
					end
		...
		elseif (Prot_Configuration[ABILITY_OVERPOWER_PROT]
			and ProtOverpower
			and Weapon()
			and UnitMana("player") >= 5
			and (ActiveStance() == 1 or (UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"]) and Prot_Configuration["PrimaryStance"] ~= 0))
			and SpellReady(ABILITY_OVERPOWER_PROT)) then
			if (ActiveStance() == 1) then
				Debug("Overpower");
				if (ActiveStance() ~= 1) then
					ProtDanceDone = true;
				end
				CastSpellByName(ABILITY_OVERPOWER_PROT);
				ProtLastSpellCast = GetTime();
			else
				if (ProtLastStanceCast + 1.5 <= GetTime()) then
					if (not ProtOldStance) then
		...
		elseif (Prot_Configuration[ABILITY_PUMMEL_PROT]
			and ProtSpellInterrupt
			and UnitMana("player") >= 10
			and (not UnitIsPlayer("target") or (UnitIsPlayer("target") and (UnitClass("target") ~= CLASS_ROGUE_PROT and UnitClass("target") ~= CLASS_WARRIOR_PROT and UnitClass("target") ~= CLASS_HUNTER_PROT)))
			and (ActiveStance() == 3 or (UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"]) and Prot_Configuration["PrimaryStance"] ~= 0))
			and SpellReady(ABILITY_PUMMEL_PROT)) then
			if (ActiveStance() == 3) then
				Debug("Pummel");
				if (ActiveStance() ~= 3) then
					ProtDanceDone = true;
				end
				CastSpellByName(ABILITY_PUMMEL_PROT);
				ProtLastSpellCast = GetTime();
			else
				if (ProtLastStanceCast + 1.5 <= GetTime()) then
					if (not ProtOldStance) then
		...
		elseif (Prot_Configuration[ABILITY_SHIELD_BASH_PROT]
			and ProtSpellInterrupt
			and UnitMana("player") >= 10
			and Shield()
			and (not UnitIsPlayer("target") or (UnitIsPlayer("target") and (UnitClass("target") ~= CLASS_ROGUE_PROT and UnitClass("target") ~= CLASS_WARRIOR_PROT and UnitClass("target") ~= CLASS_HUNTER_PROT)))
			and (ActiveStance() ~= 3 or (UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"]) and Prot_Configuration["PrimaryStance"] ~= 0))
			and SpellReady(ABILITY_SHIELD_BASH_PROT)) then
			if (ActiveStance() ~= 3) then
				Debug("Pummel");
				if (ActiveStance() == 3) then
					ProtDanceDone = true;
				end
				CastSpellByName(ABILITY_SHIELD_BASH_PROT);
				ProtLastSpellCast = GetTime();
			else
				if (ProtLastStanceCast + 1.5 <= GetTime()) then
					if (not ProtOldStance) then
		...
		elseif (Prot_Configuration[ABILITY_HAMSTRING_PROT]
			and (UnitIsPlayer("target") or (Prot_Runners[UnitName("target")] and (UnitHealth("target") / UnitHealthMax("target") * 100) <= tonumber(Prot_Configuration["HamstringHealth"])))
			and Weapon()
			and not SnareDebuff()
			and UnitMana("player") >= HamstringCost()
			and (ActiveStance() ~= 2 or (UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"]) and Prot_Configuration["PrimaryStance"] ~= 0))
			and SpellReady(ABILITY_HAMSTRING_PROT)) then
			if (ActiveStance() ~= 2) then
				Debug("Hamstring");
				if (ProtOldStance == 2) then
					ProtDanceDone = true;
				end
				CastSpellByName(ABILITY_HAMSTRING_PROT);
				ProtLastSpellCast = GetTime();
			else
				if (ProtLastStanceCast + 1.5 <= GetTime()) then
					if (not ProtOldStance) then
		...
		elseif (Prot_Configuration[ABILITY_REND_PROT]
			and UnitIsPlayer("target")
			and Weapon()
			and not AntiStealthDebuff()
			and (UnitClass("target") == CLASS_ROGUE_PROT or UnitClass("target") == CLASS_HUNTER_PROT)
			and UnitMana("player") >= 10
			and (ActiveStance() ~= 3 or (UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"]) and Prot_Configuration["PrimaryStance"] ~= 0))
			and SpellReady(ABILITY_REND_PROT)) then
			if (ActiveStance() ~= 3) then
				Debug("Rend");
				if (ProtOldStance == 3) then
					ProtDanceDone = true;
				end
				CastSpellByName(ABILITY_REND_PROT);
				ProtLastSpellCast = GetTime();
			else
				if (ProtLastStanceCast + 1.5 <= GetTime()) then
					if (not ProtOldStance) then
		...
		elseif (Prot_Configuration[ABILITY_BERSERKER_RAGE_PROT]
			and ProtBerserkerRage
			and not UnitIsPlayer("target")
			and UnitName("target") ~= "Magmadar"
			and UnitMana("player") <= Prot_Configuration["MaximumRage"]
			and (ActiveStance() == 3 or (UnitMana("player") <= ProtTacticalMastery and Prot_Configuration["PrimaryStance"] ~= 0))
			and SpellReady(ABILITY_BERSERKER_RAGE_PROT)) then
			if (ActiveStance() == 3) then
				Debug("Berserker Rage");
				if (ProtOldStance ~= 3) then
		...
		elseif (Prot_Configuration["PrimaryStance"]
			and not ProtOldStance
			and not ProtDanceDone
			and ProtLastStanceCast + 1.5 <= GetTime()
			and Prot_Configuration["PrimaryStance"] ~= ActiveStance()
			and UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"])
			and Prot_Configuration["PrimaryStance"] ~= 0) then
			--Initiate stance dance
			Debug("Primary Stance (" .. Prot_Configuration["PrimaryStance"] .. ")");
			CastShapeshiftForm(Prot_Configuration["PrimaryStance"]);
			ProtLastStanceCast = GetTime();

		elseif (Prot_Configuration[ABILITY_BLOODRAGE_PROT]
			and UnitMana("player") <= tonumber(Prot_Configuration["MaximumRage"])
			and (UnitHealth("player") / UnitHealthMax("player") * 100) >= tonumber(Prot_Configuration["BloodrageHealth"])
			and SpellReady(ABILITY_BLOODRAGE_PROT)) then
			Debug("Bloodrage");
			CastSpellByName(ABILITY_BLOODRAGE_PROT);
			--ProtLastSpellCast = GetTime();
			--No global cooldown, added anyway to ensure better flow
		
		elseif (Prot_Configuration[ABILITY_BATTLE_SHOUT_PROT]
			and not HasBuff("player", "Ability_Warrior_BattleShout")
			and UnitMana("player") >= 10
			and SpellReady(ABILITY_BATTLE_SHOUT_PROT)) then
			Debug("Battle Shout");
			CastSpellByName(ABILITY_BATTLE_SHOUT_PROT);
			ProtLastSpellCast = GetTime();

		elseif (Prot_Configuration[RACIAL_BERSERKING_PROT]
			and UnitMana("player") >= 5
			and (UnitHealth("player") / UnitHealthMax("player") * 100) <= tonumber(Prot_Configuration["BerserkHealth"])
			and not HasBuff("player", "Racial_Berserk")
			and SpellReady(RACIAL_BERSERKING_PROT)) then
			Debug("Berserking");
			CastSpellByName(RACIAL_BERSERKING_PROT);
			ProtLastSpellCast = GetTime();

		elseif (Prot_Configuration[ABILITY_DISARM_PROT]
			and Weapon()		
			and UnitIsPlayer("target")
			and (UnitClass("target") == CLASS_HUNTER_PROT or UnitClass("target") == CLASS_PALADIN_PROT or UnitClass("target") == CLASS_ROGUE_PROT or UnitClass("target") == CLASS_SHAMAN_PROT or UnitClass("target") == CLASS_WARRIOR_PROT)
			and UnitMana("player") >= 20
			and (ActiveStance() == 2 or (UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"]) and Prot_Configuration["PrimaryStance"] ~= 0))
			and SpellReady(ABILITY_DISARM_PROT)) then
			if (ActiveStance() == 2) then
				Debug("Disarm");
				if (ProtOldStance ~= 2) then
					ProtDanceDone = true;
				end				
				CastSpellByName(ABILITY_DISARM_PROT);
				ProtLastSpellCast = GetTime();
			else
				if (ProtLastStanceCast + 1.5 <= GetTime()) then
					if (not ProtOldStance) then
		...
		elseif (Prot_Configuration[ABILITY_DEMORALIZING_SHOUT_PROT]
			...
			and SpellReady(ABILITY_DEMORALIZING_SHOUT_PROT)) then
			Debug("Demoralizing Shout");
			CastSpellByName(ABILITY_DEMORALIZING_SHOUT_PROT);
			ProtLastSpellCast = GetTime();

		elseif (Prot_Configuration[ABILITY_MORTAL_STRIKE_PROT]
			and ProtMortalStrike
			and Weapon()
			and UnitMana("player") >= 30
			and SpellReady(ABILITY_MORTAL_STRIKE_PROT)) then
			Debug("Mortal Strike");
			CastSpellByName(ABILITY_MORTAL_STRIKE_PROT);
			ProtLastSpellCast = GetTime();		

		elseif (Prot_Configuration[ABILITY_BLOODTHIRST_PROT]
			and ProtBloodthirst
			and UnitMana("player") >= 30
			and SpellReady(ABILITY_BLOODTHIRST_PROT)) then
			Debug("Bloodthirst");
			CastSpellByName(ABILITY_BLOODTHIRST_PROT);
			ProtLastSpellCast = GetTime();

		elseif (Prot_Configuration[ABILITY_SHIELD_SLAM_PROT]
			and ProtShieldSlam
			and UnitMana("player") >= 20
			and SpellReady(ABILITY_SHIELD_SLAM_PROT)) then
			Debug("SHield Slam");
			CastSpellByName(ABILITY_SHIELD_SLAM_PROT);
			ProtLastSpellCast = GetTime();
		
		elseif (Prot_Configuration[ABILITY_WHIRLWIND_PROT]
			and CheckInteractDistance("target", 2)
			and UnitMana("player") >= 25
			and (ActiveStance() == 3 or (UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"]) and Prot_Configuration["PrimaryStance"] ~= 0))
			and SpellReady(ABILITY_WHIRLWIND_PROT)) then
			if (ActiveStance() == 3) then
				Debug("Whirlwind");
				if (ProtOldStance ~= 3) then
					ProtDanceDone = true;
				end				
				CastSpellByName(ABILITY_WHIRLWIND_PROT);
				ProtLastSpellCast = GetTime();
			else
				if (ProtLastStanceCast + 1.5 <= GetTime()) then
					if (not ProtOldStance) then
		...
		elseif (((Prot_Configuration[ABILITY_MORTAL_STRIKE_PROT] and ProtMortalStrike and not SpellReady(ABILITY_MORTAL_STRIKE_PROT)) or not Prot_Configuration[ABILITY_MORTAL_STRIKE_PROT] or not ProtMortalStrike) and ((Prot_Configuration[ABILITY_BLOODTHIRST_PROT] and ProtBloodthirst and not SpellReady(ABILITY_BLOODTHIRST_PROT)) or not Prot_Configuration[ABILITY_BLOODTHIRST_PROT] or not ProtBloodthirst) and ((Prot_Configuration[ABILITY_WHIRLWIND_PROT] and not SpellReady(ABILITY_WHIRLWIND_PROT)) or not Prot_Configuration[ABILITY_WHIRLWIND_PROT])) then
			--Will try to lessen the amounts of Heroic Strike, when instanct attacks (MS, BT, WW) are enabled
			if (Prot_Configuration[ABILITY_HEROIC_STRIKE_PROT]
				and Weapon()
				and UnitMana("player") >= ProtHeroicStrikeCost
				and UnitMana("player") >= Prot_Configuration["NextAttackRage"]
				and SpellReady(ABILITY_HEROIC_STRIKE_PROT)) then
				Debug("Heroic Strike");
			...
			elseif (Prot_Configuration[ABILITY_CLEAVE_PROT]
				and Weapon()
				and UnitMana("player") >= 20
				and UnitMana("player") >= Prot_Configuration["NextAttackRage"]
				and SpellReady(ABILITY_CLEAVE_PROT)) then
				Debug("Cleave");
				CastSpellByName(ABILITY_CLEAVE_PROT);
				--ProtLastSpellCast = GetTime();
				--No global cooldown, added anyway to prevent Cleave from being spammed over other abilities
			
			end

		elseif (ProtDanceDone
			and ProtOldStance
			and ProtLastStanceCast + 1.5 <= GetTime()
			and UnitMana("player") <= (ProtTacticalMastery + Prot_Configuration["StanceChangeRage"])) then
			--Initiate stance dance
			if (not Prot_Configuration["PrimaryStance"]) then
				Debug("Old Stance (" .. ProtOldStance .. ")");
				CastShapeshiftForm(ProtOldStance);
			else
				Debug("Primary Stance (" .. Prot_Configuration["PrimaryStance"] .. ")");
				CastShapeshiftForm(Prot_Configuration["PrimaryStance"]);
			end
			ProtLastStanceCast = GetTime();
			if (ProtOldStance == ActiveStance() or Prot_Configuration["PrimaryStance"] == ActiveStance()) then
